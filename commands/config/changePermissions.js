...
async function verifyGroupAndListPermissions(client, message) {
    ...
    if(duplicated){
        ...
        return await client.sendText(message.chatId, appName+`Ente grupo ainda não registado. \nPara registar o grupo utilize o comando /start.`);
    }
}
    ...
    if(admins.includes(message.sender.id)) {
        if (admins.includes((await client.getHostNumber()).toString() + '@c.us')) {
            ...
            if (permissionsValid.length === 0) {
                ...
                const duplicated = await Group.findOne({chatId: groupId}).exec();
                if (duplicated) {
                    const result = await Group.findOneAndUpdate({chatId: groupId},{
                        chatId: groupId,
                        rules: permissionsValid,
                        createdBy: message.sender.id,
                    }).exec();
                    if (result) {
                        return await client.sendText(message.chatId, appName + `Permissões atualizadas com sucesso, irei seguir as novas indicações👮🏻‍♀️.`);
                    } else {
                        return await client.sendText(message.chatId, appName + `Ocorreu um erro ao registar o grupo.`);
                    }
                }else{
                    return await client.sendText(message.chatId, appName + `O grupo ainda não está registado. \nPara registar o grupo utilize o comando /start.`);
                }
            }
        } else {
            return await client.sendText(message.chatId, appName + `Para continuar a configuração tem de me colocar como administrador do grupo.`);
...
const signature={
    ...
    onlyGroup: false,
    onlyPrivate: true,
    description: "Alterar as permissões do grupo",
    handler
}
